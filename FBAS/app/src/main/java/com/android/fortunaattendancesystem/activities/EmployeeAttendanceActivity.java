...
//                }, 500);
                                                ...
                                                if (status) {
                                                    ...
                                                    if (firstFingerTempLen == Constants.TEMPLATE_SIZE && secondFingerTempLen == Constants.TEMPLATE_SIZE) { //Two Valid Templates Found
                                                        ...
                                                        if (morphoDevice != null && morphoDatabase != null) {
                                                            ...
                                                            initFingerVerificationByCard(cardDetails);//Start Finger Verification Using Card
                                                        } else {
                                                            runOnUiThread(new Runnable() {
                                                                @Override
                                                                public void run() {
                                            ...
                                            if (data != null && data.length > 0) {
                                                ...
                                                if (arr != null && arr.length == 3) {
                                                    ...
                                                    if (!strData.equals("RD-FAIL")) {
                                                        cardInfo.setCardId(Utility.hexToAscii(strData.substring(0, 16)));
                                                        status = readCardTemplateUsingRC522(comm, cardInfo);
                                                        if (status) {
                                                            MorphoDevice morphoDevice = ProcessInfo.getInstance().getMorphoDevice();
                                                            MorphoDatabase morphoDatabase = ProcessInfo.getInstance().getMorphoDatabase();
                                                            if (morphoDevice != null && morphoDatabase != null) {
                                                                ...
                                                                if (fft != null && sft != null) {//Two finger template found
                                                                    ...
                                                                    if (l1 == Constants.TEMPLATE_SIZE && l2 == Constants.TEMPLATE_SIZE) { //Two Valid Templates Found
                                                                        ...
                                                                        initFingerVerificationByCard(cardInfo);//Start Finger Verification Using Card
                                                                    } else {
                                                                        runOnUiThread(new Runnable() {
                                                                            @Override
                                                                            public void run() {
                                                                                ...
                                                                                showFailureCustomDialog("Card Read Status", "Invalid Template Length ! Put Card Again");
                                                                                pHandler.postDelayed(new Runnable() {
                                                                                    @Override
                                                                                    public void run() {
                                                                                        failureEmpDetailsDialog.cancel();
    ...
    private void initFingerVerificationByCard(final SmartCardInfo cardDetails) {
                                                ...
                                                if (info != null) {
                                                    ...
                                                    switch (val) {
                                                        case "Y":
                                                            ...
                                                            try {
                                                                int wigValue = Integer.parseInt(rawData, 2);
                                                                isValid = true;
                                                            } catch (NumberFormatException ne) {
                                                            }
                                                            if (isValid) {
...
//        };
    ...
    private void initFVByLocalDb(EmployeeInfo empInfo) {
...
//        }
    ...
    private void showPasswordDialog() {
        ...
        dialog.setContentView(R.layout.username_password_dialog);

        TextView title = (TextView) dialog.findViewById(R.id.title);

        final EditText et_Username = (EditText) dialog.findViewById(R.id.etUsername);
                    ...
                    if (userData != null) {
                        if (userData.getCount() == 1) {
                            while (userData.moveToNext()) {
                                ...
                                boolean isIdentificationStarted = ProcessInfo.getInstance().isIdentificationStarted();
                                boolean isVerificationStarted = ProcessInfo.getInstance().isVerificationStarted();
                                if (isIdentificationStarted || isVerificationStarted) {
                                    morphoDevice = ProcessInfo.getInstance().getMorphoDevice();
                                    morphoDatabase = ProcessInfo.getInstance().getMorphoDatabase();
                                    if (morphoDevice != null && morphoDatabase != null) {
                                        boolean isBioCommandStarted = ProcessInfo.getInstance().isCommandBioStart();
                                        if (isBioCommandStarted) {
                                            morphoComm.stopFingerIdentification();
                                        }
                                    }
                                }
                        ...
                        showCustomAlertDialog(false, "Invalid User", "Please check username and password !");
                    }
                }
            }
        });
        ...
        if (value == 0) {
            ...
            if (morphoDevice != null && morphoDatabase != null) {
              ...
              //  Log.d("TEST","************************************************");
                ...
                showCustomAlertDialog(false, "Device Connection Status", "Finger Reader Not Found");
            }
        }
    }
