...
//                                                    if (EmployeeAttendanceActivity.hasCamera) {
//                                                        if (EmployeeAttendanceActivity.safeToTakePicture) {
//                                                            if (EmployeeAttendanceActivity.mCamera != null) {
//                                                                EmployeeAttendanceActivity.mCamera.takePicture(null, null, mCall);
//                                                            }
//                                                            EmployeeAttendanceActivity.safeToTakePicture = false;
//                                                        }
//                                                    }
...
//                                                                        SQLiteCommunicator empInfoList.add(emInfo); dbComm = new SQLiteCommunicator();
//                                                                        AttendanceInfo attInfo = new AttendanceInfo();
//                                                                        byte[] pictureData = EmployeeAttendanceActivity.pictureData;
//                                                                        if (pictureData != null && pictureData.length > 0) {
//
//                                                                            String photo = Base64.encodeToString(pictureData, Base64.DEFAULT);
//
//                                                                            Date date = new Date();
//                                                                            String strDateFormat = "dd-MM-yyyy/HHmmss";
//                                                                            DateFormat dateFormat = new SimpleDateFormat(strDateFormat);
//                                                                            String dateTime = dateFormat.format(date);
//
//                                                                            String arrDateTime[] = dateTime.split("/");
//
//                                                                            if (arrDateTime != null && arrDateTime.length == 2) {
//
//                                                                                String strPunchDate = "";
//                                                                                String strPunchTime = "";
//
//                                                                                strPunchDate = arrDateTime[0].replaceAll("-", "").trim();
//                                                                                String datex = strPunchDate.substring(0, 2);
//                                                                                String month = strPunchDate.substring(2, 4);
//                                                                                String year = strPunchDate.substring(4);
//
//                                                                                String hr = arrDateTime[1].substring(0, 2);
//                                                                                String min = arrDateTime[1].substring(2, 4);
//                                                                                String sec = arrDateTime[1].substring(4, 6);
//                                                                                String tempPunchTime = hr + ":" + min + ":" + sec;
//
//                                                                                strPunchDate = datex + "/" + month + "/" + year;
//                                                                                strPunchTime = tempPunchTime; //finalDateFormat.format(initialDateFormat.parse(strPunchTime));
//
//                                                                                attInfo.setEmpId(morphoUser.getField(0));
//                                                                                attInfo.setLatLong("");
//                                                                                attInfo.setImageLen(photo.length());
//                                                                                attInfo.setImageBase64(photo);
...
//                                                                                String payload = JSONCreatorParser.getAttendanceJsonData(pid, imei, Constants.CORPORATE_ID, deviceToken, Constants.ATTENDANCE_POST_COMM, attInfo);
//                                                                                if (payload != null && payload.trim().length() > 0) {
//                                                                                    MqttApi mqttApi = new MqttApi(context);
//                                                                                    MqttMessage message = new MqttMessage();
//                                                                                    message.setPayload(payload.getBytes());
//                                                                                    try {
//                                                                                        String attendancePubTopic = Constants.ATTENDANCE_RECV_TOPIC + Constants.OU + "/" + Constants.DEV_TYPE + "/" + Constants.DEV_COMP_NAME + "/CTS/" + imei;
//                                                                                        try {
//                                                                                            mqttApi.publish(mqttClient, attendancePubTopic, message);
...
//                                                                }
    ...
    boolean isVerifyEndProcessCalled = false;

    private void notifyVerificationEndProcess() {
        if (!isVerifyEndProcessCalled) {
            isVerifyEndProcessCalled = true;
            mHandler.post(new Runnable() {
                @Override
                public synchronized void run() {
                    try {
    ...
    private void stopProcess(boolean reStartIdentification) {
        if (ProcessInfo.getInstance().isStarted()) {
            stop(reStartIdentification);
        }
    }

    //================================= stop process  =======================================//

    private void stop(boolean reStartIdentification) {
        if (reStartIdentification) {
                    ...
                    if (!isIdentificationStarted && !isVerificationStarted && !isBioCommandStarted) {
                        pHandler.postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                EmployeeAttendanceActivity.stopHandler();
                                EmployeeAttendanceActivity.startHandler();
                                isVerifyEndProcessCalled=false;
                            }
                        }, 500);//1500//1000
                    }
                } else {
                                                    ...
                                                    if (fingerId == -1) {
                                                        ...
                                                        if (empFingerId != -1) {
                                                            ...
                                                            if (fingerId != -1) {
                                                                ...
                                                                if (strVerificationMode.equals("CARD+PIN+FINGER")) {
                                                                    String pin = empFingerInfo.getPin();
                                                                    if (pin != null && pin.trim().length() > 0) {
                                                                        int status = dbComm.insertCardVerificationPin(empAutoId, pin);
                                                                        if (status != -1) {
                                                                            showCustomAlertDialog(R.drawable.success, "Finger Save Status", "Finger Data Saved Successfully", true);
                                                                        } else {
                                                                            showCustomAlertDialog(R.drawable.failure, "Finger Save Status", "Failed To Save Card Pin Data", true);
                                                                        }
                                                                    }
                                                                } else {
                                                                    showCustomAlertDialog(R.drawable.success, "Finger Save Status", "Finger Data Saved Successfully", true);
                                                        ...
                                                        if (status != -1) {
                                                            ...
                                                            if (status != -1) {
                                                                if (strVerificationMode.equals("CARD+PIN+FINGER")) {
                                                                    String pin = empFingerInfo.getPin();
                                                                    if (pin != null && pin.trim().length() > 0) {
                                                                        status = dbComm.insertCardVerificationPin(empAutoId, pin);
                                                                        if (status != -1) {
                                                                            showCustomAlertDialog(R.drawable.success, "Finger Save Status", "Finger Data Saved Successfully", true);
                                                                        } else {
                                                                            showCustomAlertDialog(R.drawable.failure, "Finger Save Status", "Failed To Save Card Pin Data", true);
                                                            ...
                                                            if (strVerificationMode.equals("CARD+PIN+FINGER")) {
                                                                ...
                                                                if (pin != null && pin.trim().length() > 0) {
                                                                    ...
                                                                    if (status != -1) {
                                                                        ...
                                                                        int smartReader = Settings.getInstance().getSrTypeValue();
                                                                        if (role.equals("Y") && (smartReader == 0 || smartReader == 1 || smartReader == 2)) {//0=RC632,1=MicroSmartV2//2=RC522
                                                                            cardWriteConfirmDialog(R.drawable.success, "Finger Save Status", "Finger And Pin Data Saved Successfully ! Do You Want To Write Data Into Smart Card ?");
                                                                        } else {
                                                                            showCustomAlertDialog(R.drawable.success, "Finger Save Status", "Finger Data Saved Successfully", true);
                                                                        }
                                                                    } else {
